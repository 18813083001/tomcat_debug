...
public class Client {
    ...
    org.slf4j.Logger slfLogger = LoggerFactory.getLogger(Client.class);
...
//        System.exit(exitCode);
    ...
    throws IOException {
        ...
        Map credentials = formatCredentials(login, password);
        if (credentials != null){
            environment.putAll(credentials);
        }
        long period = getConnectionHeartbeatPeriod();
        if (period > 0){
            environment.put(EnvHelp.CLIENT_CONNECTION_CHECK_PERIOD,String.valueOf(period));
        }
...
//                        logger.info(rs);
    ...
    public Object [] execute(final String hostport, final String login,
...
//        }
     ...
     * 发生异常后，关闭连接，子类实现具体操作
     * */
    public void closeJmxConnection(final String hostport, final String login,
                                   final String password){}
    /**
     * 单个连接的心跳时间,毫秒
     * @return 如果为0，则会采用默认的心跳时间，60秒一次
     * */
